...
```
### All components togehter
```{r}
movement_long <- reshape2::melt(movement_fmri %>% select(-rotation_sum, -rotation_x_sum, -rotation_y_sum), id.vars=c("pulse_id", "participant", "movement_type", "session"))
...
for(component_name in component_names){
  ...
  eval(parse(text=
     paste0("ez_res <- ezANOVA(movement_fmri,
        dv =", component_name,",
        wid = participant,
        within = movement_type,
        return_aov=TRUE)")))
  print(summary(ez_res$aov))
  print(ggplot(movement_fmri, aes(participant, .data[[component_name]], fill=movement_type)) + geom_boxplot())
  print(ggplot(movement_fmri, aes(movement_type, .data[[component_name]])) + geom_boxplot())
}
```