...
onset_stop_table.participants <- function(participants, speed_threshold, still_threshold, 
  ...
  for(participant_name in names(participants)){
    ...
    participant_results <- onset_stop_table.participant(participants[[participant_name]], speed_threshold = speed_threshold, 
                                                        still_threshold = still_threshold, min_duration = min_duration, 
                                                        still_duration = still_duration, pause_duration = pause_duration)
...
onset_stop_table.participant <- function(participant, speed_threshold, still_threshold, 
  ...
  for(i in 1:length(participant)){
    ...
    session_results <- onset_stop_table.session(session_data, speed_threshold = speed_threshold, 
                                                still_threshold = still_threshold, min_duration = min_duration, 
                                                still_duration = still_duration, pause_duration = pause_duration)
