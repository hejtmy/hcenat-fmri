...
#'
#' @examples
onset_stop_table.participants <- function(participants, speed_threshold, still_threshold, min_duration, pause_duration){
  out <- data.frame()
  for(participant_name in names(participants)){
    message("calculating for ", participant_name)
    participant_results <- onset_stop_table.participant(participants[[participant_name]], speed_threshold, 
                                                        still_threshold, min_duration, pause_duration)
...
#'
#' @examples
onset_stop_table.participant <- function(participant, speed_threshold, still_threshold, min_duration, pause_duration){
  out <- data.frame()
  for(i in 1:length(participant)){
    session_data <- participant[[i]]
    if(is.null(session_data)) next
    session_results <- onset_stop_table.session(session_data, speed_threshold, still_threshold, min_duration, pause_duration)
