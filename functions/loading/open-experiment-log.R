open_experiment_log <- function(filepath){
  ls <- list()
  text <- readLines(filepath,warn=F)#reads into a text file at first
  idxHeaderTop <- which(grepl('\\*\\*\\*\\*\\*',text))#finds the header start
  idxHeaderBottom <- which(grepl('\\-\\-\\-\\-\\-',text))#finds the header bottom
