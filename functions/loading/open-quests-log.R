open_quests_logs <- function(experiment_log, scenario_log = NULL){
  ...
  # it looks for steps finished because of bad logging
  ...
  if (nrow(table_steps_activated) >= nrow(table_steps_finished)){
    use_finished = FALSE
  } else {
    use_finished = TRUE
  }
...
get_activated_quest_name <- function(string = ""){
  ...
  name <- substring(name, 2, nchar(name) - 1) # removing the square brackets
  return(name)
}